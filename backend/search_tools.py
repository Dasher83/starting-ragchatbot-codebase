...
class CourseSearchTool(Tool):
    ...
    def _format_results(self, results: SearchResults) -> str:
        ...
        formatted = []
        seen_labels = {}  # label -> source dict, for deduplication
        ...
        for doc, meta in zip(results.documents, results.metadata):
            ...
            # Build source label and look up its link
            label = course_title
            if lesson_num is not None:
            ...
            if label not in seen_labels:
                ...
                seen_labels[label] = {"label": label, "url": url}

            formatted.append(f"{header}\n{doc}")

        # Store deduplicated sources for retrieval
        self.last_sources = list(seen_labels.values())

        return "\n\n".join(formatted)

class CourseOutlineTool(Tool):
    """Tool for retrieving a full course outline"""
        ...
        return {
            ...
            "input_schema": {
                ...
                "properties": {
                    "course_title": {
                        "type": "string",
                        "description": "Course name or partial name (e.g. 'MCP', 'Introduction to Python')"
                    }
                },
